...
void AddRadialGradientVisuals(ImDrawList* draw_list, const ImVec2& center, float radius, ImU32 col_in, ImU32 col_out)
{
...
void Visuals::visualizeSpread(ImDrawList* drawList) noexcept
{
    ...
    if (ImVec2 edge; Helpers::worldToScreen(local.inaccuracy, edge))
    {
        ...
        AddRadialGradientVisuals(drawList, displaySize / 2, radius, Helpers::calculateColor(config->visuals.spreadCircle, 0.0f), color);
    }
}
