...
void distortion(UserCmd* cmd) {
    auto moving_flag{ get_moving_flag(cmd) };
    auto speed = config->rageAntiAim[static_cast<int>(moving_flag)].distortionSpeed;
    auto amount = config->rageAntiAim[static_cast<int>(moving_flag)].distortionAmount;
    if (!speed)
        speed = RandomFloat(0.f, 100.f, 1.f);
    if (!amount)
        amount = RandomFloat(0.f, 360.f, 1.f);
    const float sine = ((sin(memory->globalVars->currenttime * (speed / 10.f)) + 1) / 2) * amount;
...
void AntiAim::JitterMove(UserCmd* cmd) {
    ...
    if (abs(cmd->sidemove) + abs(cmd->forwardmove) < 10.f)
        return;

    if (localPlayer->m_vecVelocity().length2DSqr() < 140.f * 140.f)
        return;

    float factor = 0.95f + fmodf(memory->globalVars->currenttime, 0.2f) * 0.25f;
    ...
    if (cmd->viewangles.y == currentViewAngles.y || Tickbase::isShifting())
    {
        if (config->rageAntiAim[static_cast<int>(moving_flag1)].yawBase != Yaw::off)   //AntiAim
        {
            ...
            case 1: //Jitter centered
                if (!isManualSet && isFreestanding == false && !willgetstabbed)
                {
                    if (config->rageAntiAim[static_cast<int>(moving_flag1)].desync && config->rageAntiAim[static_cast<int>(moving_flag1)].peekMode == 3)
                        yaw -= invert ? (memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].jitterMin, config->rageAntiAim[static_cast<int>(moving_flag1)].jitterRange)) : -(memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].jitterMin, config->rageAntiAim[static_cast<int>(moving_flag1)].jitterRange));
                    else
                        yaw -= flipJitter ? (memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].jitterMin, config->rageAntiAim[static_cast<int>(moving_flag1)].jitterRange)) : -(memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].jitterMin, config->rageAntiAim[static_cast<int>(moving_flag1)].jitterRange));
                }
                break;
            case 2: // jitter offset
                if (!isManualSet && isFreestanding == false && !willgetstabbed)
                {
                    yaw += flipJitter ? config->invert.isActive() ? (memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].jitterMin, config->rageAntiAim[static_cast<int>(moving_flag1)].jitterRange)) : -(memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].jitterMin, config->rageAntiAim[static_cast<int>(moving_flag1)].jitterRange)) : 0;
                }
                break;
            case 3:
                if (!isManualSet && isFreestanding == false && !willgetstabbed)
            ...
            case 6: //spin           
                if (!isManualSet && isFreestanding == false && !willgetstabbed)
                {
                    yaw = -180.0f + (cmd->tickCount % 9) * config->rageAntiAim[static_cast<int>(moving_flag1)].spinBase;
                }
                break;
            default:
                break;
            ...
            if (!isManualSet && isFreestanding == false && !willgetstabbed)
                if (config->rageAntiAim[static_cast<int>(moving_flag1)].distortion)
                    distortion(cmd);
        ...
        if (config->rageAntiAim[static_cast<int>(moving_flag1)].desync && !Tickbase::isShifting())
        {
            ...
            float leftDesyncAngle = memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].leftMin, config->rageAntiAim[static_cast<int>(moving_flag1)].leftLimit) * 2.f;
            float rightDesyncAngle = memory->randomFloat(config->rageAntiAim[static_cast<int>(moving_flag1)].rightMin, config->rageAntiAim[static_cast<int>(moving_flag1)].rightLimit) * -2.f;
