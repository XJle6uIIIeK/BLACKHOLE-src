...
{
    float latency = 0.0f;
    if (auto networkChannel = interfaces->engine->getNetworkChannel(); networkChannel && networkChannel->getLatency(0) > 0.0f && !config->backtrack.fakeLatency) {
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); i++)
    {
        ...
        if (records.simulationTime != entity->simulationTime() && records.simulationTime < entity->simulationTime())
        {
            ...
            records.simulationTime != entity->simulationTime() ?
                records.chokedPackets = static_cast<int>(simDifference / memory->globalVars->intervalPerTick) - 1 : records.chokedPackets = 0;
            ...
            else
            {
                ...
                records.velocity.z -= gravity->getFloat() * 0.5f * ticksToTime(records.chokedPackets);
            }

            if (entity->flags() & FL_ONGROUND
                && records.layers[ANIMATION_LAYER_ALIVELOOP].weight > 0.f
                && records.layers[ANIMATION_LAYER_ALIVELOOP].weight < 1.f
                && records.layers[ANIMATION_LAYER_ALIVELOOP].cycle > records.oldlayers[ANIMATION_LAYER_ALIVELOOP].cycle)
            {
            ...
            if ((records.layers[ANIMATION_LAYER_MOVEMENT_MOVE].playbackRate <= 0.f
                || records.layers[ANIMATION_LAYER_MOVEMENT_MOVE].weight <= 0.f)
            ...
            if (Entity* p_weapon = entity->getActiveWeapon(); p_weapon && p_weapon->getWeaponData())
                max_speed = std::fmaxf(entity->isScoped() ? p_weapon->getWeaponData()->maxSpeedAlt : p_weapon->getWeaponData()->maxSpeed, 0.001f);
        ...
        if (runPostUpdate)
        {
            ...
            while (records.backtrackRecords.size() > 3 && records.backtrackRecords.size() > static_cast<size_t>(timeToTicks(timeLimit)))
