...
std::uintptr_t Memory::findPattern(const char* moduleName, std::string_view pattern, bool reportNotFound = true) noexcept
{
    ...
    if(reportNotFound)
        MessageBoxA(NULL, (std::string(skCrypt("Failed to find pattern #")) + std::to_string(id) + '!').c_str(), skCrypt("BLACKHOLE"), MB_OK | MB_ICONWARNING);
    return 0;
}

Memory::Memory() noexcept
{
    present = findPattern(skCrypt("gameoverlayrenderer").decrypt(), skCrypt("\x8B\x4D?\xA1????\x51\xFF").decrypt()) + 4;
    reset = findPattern(skCrypt("gameoverlayrenderer").decrypt(), skCrypt("\xE8????\xA1????\x57\x53").decrypt()) + 6;
    ...
    msg_voice_data = findPattern(ENGINE_DLL, "\x55\x8B\xEC\x83\xE4\xF8\xA1????\x81\xEC\x84\x01\x00");
