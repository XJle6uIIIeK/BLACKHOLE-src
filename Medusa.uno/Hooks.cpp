...
static void __fastcall runCommand(void* ecx, void* edx, Entity* entity, UserCmd* cmd, MoveHelper* moveHelper)
{
    static auto original = hooks->prediction.getOriginal<void, 19>(entity, cmd, moveHelper);
    if (!localPlayer || !entity || entity != localPlayer.get())
        return;
    ...
    Tickbase::getCorrectTickbase(cmd->commandNumber);

    const int backup_tickbase = tickbase;
    const float backup_velocity_modifier = localPlayer->velocityModifier();

    original(ecx, localPlayer.get(), cmd, moveHelper);

    localPlayer->velocityModifier() = backup_velocity_modifier;
}

static bool __fastcall msg_voices_data(void* ecx, void* edx, c_svc_msg_voice_data* message)
{
    // получаем оригинал функции
    static auto original = hooks->msg_voice_data.getOriginal<bool>(message);

    // исключаем локального игрока т.к нам нужны данные соперников ну или тиммейтов
    if (!localPlayer || localPlayer->index() == message->client + 1)
        return original(ecx, message);
...
void Hooks::install() noexcept
{
    ...
    **reinterpret_cast<decltype(reset)***>(memory->reset) = reset;
    ...
    msg_voice_data.detour(memory->msg_voice_data, msg_voices_data);
