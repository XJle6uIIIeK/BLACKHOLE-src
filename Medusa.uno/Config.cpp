...
{
    CreateDirectoryA(skCrypt("C:\\BLACKHOLE\\"), NULL);
    path /= std::string(skCrypt("C:\\BLACKHOLE\\Configs"));
    listConfigs();
    misc.clanTag[0] = '\0';
    misc.name[0] = '\0';
...
static void from_json(const json& j, Config::RageAntiAimConfig& a)
{
    ...
    read(j, "Tick Delays", a.tickDelays);
    ...
    read(j, "Distortion", a.distortion);
    read(j, "Distortion Speed", a.distortionSpeed);
    read(j, "Distortion Amount", a.distortionAmount);
...
static void from_json(const json& j, Config::Visuals::Warning& gp)
{
    read(j, "enabled", gp.enabled);
    read(j, "enemiesOnly", gp.enemiesOnly);
    read(j, "flash", gp.flash);
    read(j, "HE", gp.HE);
    read(j, "smoke", gp.smoke);
    read(j, "molotov", gp.molotov);
    read(j, "decoy", gp.decoy);
...
static void from_json(const json& j, Config::Visuals& v)
{
    ...
    read(j, "GP enabled", v.GPenabled);
    read<value_t::object>(j, "GP trail", v.GPtrail);
    read<value_t::object>(j, "GP colision", v.GPcolision);
    read<value_t::object>(j, "GP ground", v.GPground);
    read<value_t::object>(j, "GP warning", v.warning);
...
static void from_json(const json& j, Config::ClanTagConfig::Custom& c)
{
...
static void from_json(const json& j, Config::ClanTagConfig& ct)
{
    read(j, "Mode", ct.mode);
...
static void from_json(const json& j, Config::Misc& m)
{
    ...
    read(j, "Null Strafe", m.nullStrafe);
    ...
    read(j, "Fakewalk", m.fakewalk);
    read(j, "Fakewalk key", m.fakeWalkKey);
    ...
    read(j, "Cheat revealer", m.cheatRevealer);
...
static void from_json(const json& j, Config::Menu& m)
{
    ...
    read<value_t::object>(j, "Color", m.lineCol);
    read(j, "Amount", m.N);
    read(j, "Max Distance", m.lineMaxDist);
    read(j, "Thickness", m.lineThickness);
    read(j, "Darkness", m.darkness);
}
...
void Config::load(const char8_t* name, bool incremental) noexcept
{
    ...
    read<value_t::object>(j, "Clantag", clanCfg);
...
static void to_json(json& j, const Config::Ragebot& o, const Config::Ragebot& dummy = {})
{
    ...
    WRITE("HB", hitboxes);
    WRITE("HC", hitChance);
    //WRITE("Hitboxes Priority", prior_hit);
...
static void to_json(json& j, const Config::ClanTagConfig::Custom& o, const Config::ClanTagConfig::Custom& dummy = {})
{
    WRITE("Prefix", prefix);
    WRITE("Tag", tag);
...
static void to_json(json& j, const Config::Misc& o)
{
    ...
    WRITE("Fakewalk", fakewalk);
    WRITE("Fakewalk key", fakeWalkKey);
    ...
    WRITE("Cheat revealer", cheatRevealer);
...
static void to_json(json& j, const Config::Visuals::Warning& o, const Config::Visuals::Warning& dummy)
{
    WRITE("enabled", enabled);
    WRITE("enemiesOnly", enemiesOnly);
    WRITE("flash", flash);
    WRITE("HE", HE);
    WRITE("smoke", smoke);
    WRITE("molotov", molotov);
    WRITE("decoy", decoy);
...
static void to_json(json& j, const Config::Visuals& o)
{
    ...
    WRITE("GP enabled", GPenabled);
    WRITE("GP trail", GPtrail);
    WRITE("GP colision", GPcolision);
    WRITE("GP ground", GPground);
    WRITE("GP warning", warning);
}
