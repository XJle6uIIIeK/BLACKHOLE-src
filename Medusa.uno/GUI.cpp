...
void darkBck(ImDrawList* d, ImVec2 b) {
    d->AddRectFilled({ 0.f, 0.f }, b, Helpers::calculateColor(Color4{ 0.f, 0.f, 0.f, config->menu.darkness }), 0.f);
}
...
{
    static std::vector<std::pair<ImVec2, ImVec2>> points(config->menu.N);
    ...
    for (int i = 0; i < config->menu.N; i++) {
        for (int j = i + 1; j < config->menu.N; j++) {
            Dist = length(points[i].first - points[j].first);
            if (Dist < config->menu.lineMaxDist) d->AddLine(points[i].first, points[j].first, Helpers::calculateColor(config->menu.lineCol), config->menu.lineThickness);
        }
    }
}// Background effect
...
void GUI::render() noexcept
{
    ...
    if (config->menu.darkness)
        darkBck(ImGui::GetBackgroundDrawList(), screenSize);

    if (config->menu.lineCol.enabled)
...
#include "Hacks/NadePredEXP.cpp"
#include "Hacks/cheat_revealer.hpp"
...
void renderLegitBotWindow() noexcept
{
    ...
    {
        ImGui::SliderInt(skCrypt("Time limit"), &config->backtrack.timeLimit, 1, static_cast<int>(Backtrack::getMaxUnlag() * 1000), "%d ms");
    }
    ImGui::PopItemWidth();
    ImGui::NextColumn();
...
void GUI::renderRageAntiAimWindow() noexcept
{
    ...
    ImGui::PopID();
    ImGui::Checkbox(skCrypt("Distortion"), &config->rageAntiAim[current_category].distortion);
    if (config->rageAntiAim[current_category].distortion) {
        ImGui::PushItemWidth(100.0f);
        ImGui::SliderFloat("Speed", &config->rageAntiAim[current_category].distortionSpeed, 0.f, 100.f, config->rageAntiAim[current_category].distortionSpeed > 0 ? "%1.fHz" : "Random Hz");
        ImGui::PopItemWidth();
        ImGui::PushItemWidth(100.0f);
        ImGui::SliderFloat("Amount", &config->rageAntiAim[current_category].distortionAmount, 0.f, 360.f, config->rageAntiAim[current_category].distortionAmount > 0 ? "%1.f" : "Random");
        ImGui::PopItemWidth();
    }
    ImGui::NextColumn();
    ...
    ImGui::SliderInt("Left min", &config->rageAntiAim[current_category].leftMin, 0, config->rageAntiAim[current_category].leftLimit - 1, "%d");

    ImGui::PopItemWidth();

    ImGui::PushItemWidth(220.0f);
    ImGui::SliderInt(skCrypt("Right limit"), &config->rageAntiAim[current_category].rightLimit, 0, 60, "%d");
    ImGui::SliderInt("Right min", &config->rageAntiAim[current_category].rightMin, 0, config->rageAntiAim[current_category].rightLimit - 1, "%d");

    ImGui::PopItemWidth();
    ImGui::PushItemWidth(220.0f);
    ImGui::Combo(c_xor("Peek Mode"), &config->rageAntiAim[current_category].peekMode, skCrypt("Off\0Peek real\0Peek fake\0Jitter\0Switch\0"));
    ImGui::Combo(c_xor("LBY mode"), &config->rageAntiAim[current_category].lbyMode, skCrypt("Normal\0Opposite\0Sway\0Fake\0"));
    ...
    ImGui::PushItemWidth(220.0f);
    ImGui::SliderInt(c_xor("Fake-lag Limit"), &config->fakelag.limit, 1, 16, "%d");
    ...
    ImGui::PopID();
    ImGui::Checkbox(skCrypt("Fakewalk"), &config->misc.fakewalk);
    ImGui::SameLine();
    ImGui::PushID(skCrypt("Fakewalk Key"));
    ImGui::hotkey2("", config->misc.fakeWalkKey);
    ImGui::PopID();
    ...
    static const char* breakers[]{ "Static legs in air", "Zero pitch on land", "No animations", "Walk in air", "Move jitter" };
    static bool breaker[ARRAYSIZE(breakers)] = { false, false, false, false , false };
    static std::string previewvalue = "";
    bool once = false;
    for (size_t i = 0; i < ARRAYSIZE(breaker); i++)
    {
        breaker[i] = (config->condAA.animBreakers & 1 << i) == 1 << i;
    }
    if (ImGui::BeginCombo(skCrypt("Animation breakers"), previewvalue.c_str()))
    {
        previewvalue = "";
        for (size_t i = 0; i < ARRAYSIZE(breakers); i++)
        {
            ImGui::Selectable(breakers[i], &breaker[i], ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
        }
        ImGui::EndCombo();
    }
    for (size_t i = 0; i < ARRAYSIZE(breakers); i++)
    {
        ...
        if (breaker[i])
        {
            previewvalue += previewvalue.size() ? std::string(", ") + breakers[i] : breakers[i];
...
void GUI::renderChamsWindow() noexcept
{
    ...
    ImGui::Combo("Material", &chams.material, "Normal\0Flat\0Animated\0Animated2\0Glass\0Chrome\0Crystal\0Plastic\0Glow\0Fractal\0Pearlescent\0Metallic\0Smoke Light\0Smoke Dark\0Water\0Bush\0MP3 Detail\0Snowflakes\0");
    ImGui::PopItemWidth();
...
void renderESPpreview(ImDrawList* draw, ImVec2 pos) noexcept
{
    ...
    auto sizeMed = ImGui::CalcTextSize(skCrypt("BLACKHOLE"));
    ImGui::PopFont();
    ImGui::PushFont(gui->weaponIcons());
    ...
    if (currentCategory == 0)
    {
        ...
        {
            draw->AddText(gui->espFont(), 12.f, ImVec2{pos.x + 185 / 2 - (sizeMed.x / 2), pos.y + 25}, Helpers::calculateColor(config->esp.enemy.name), c_xor("BLACKHOLE"));
        }
    ...
    if (currentCategory == 1)
    {
        ...
        {
            draw->AddText(gui->espFont(), 12.f, ImVec2{ pos.x + 185 / 2 - (sizeMed.x / 2), pos.y + 25 }, Helpers::calculateColor(config->esp.allies.name), c_xor("BLACKHOLE"));
        }
...
void GUI::renderVisualsWindow() noexcept
{
    ...
    ImGui::Checkbox(skCrypt("Enemy Grenade Prediction"), &config->visuals.GPenabled);
    ImGui::SameLine();
    ImGui::PushID("Grenade Prediction");
    if (ImGui::Button("..."))
        ImGui::OpenPopup("");
    ...
    if (ImGui::BeginPopup("")) {
        ImGuiCustom::colorPicker("Trail", config->visuals.GPtrail);
        ImGuiCustom::colorPicker("Colision box", config->visuals.GPcolision);
        ImGuiCustom::colorPicker("Ground radius", config->visuals.GPground);
        ImGui::Checkbox("Enable Grenade Warning", &config->visuals.warning.enabled);
        ImGui::Checkbox("Enemies only", &config->visuals.warning.enemiesOnly);
        ImGui::Checkbox("Flashbangs", &config->visuals.warning.flash);
        ImGui::Checkbox("HE grenade", &config->visuals.warning.HE);
        ImGui::Checkbox("Smoke grenade", &config->visuals.warning.smoke);
        ImGui::Checkbox("Molotov/incendiary grenade", &config->visuals.warning.molotov);
        ImGui::Checkbox("Decoy", &config->visuals.warning.decoy);
        ImGui::EndPopup();
    }
    ...
    if (config->visuals.scope.type == 2)
    {
        ...
        if (ImGui::BeginPopup("")) {
            ...
            ImGui::SliderInt(skCrypt("Speed"), &config->visuals.scope.speed, 0.0f, 100.0, skCrypt("%d"));
    ...
    {
        ImGui::SameLine();
        ImGui::PushID("?1'thirdperson'1?tf is u looking in hex for 'BLACKHOLE");
        if (ImGui::Button("..."))
            ImGui::OpenPopup("ttt");
...
void GUI::renderMovementWindow() noexcept
{
    ...
    ImGui::Checkbox("Null Strafe", &config->misc.nullStrafe);
    ...
    ImGui::SameLine();
    ImGui::PushID("stop using hex u dirty hoe, BLACKHOLE");
    if (ImGui::Button("..."))
        ImGui::OpenPopup("");
    if (ImGui::BeginPopup(""))
    {
        ...
        ImGui::Combo(c_xor("Sound"), &config->misc.ebdetect.sound, skCrypt("None\0BLACKHOLE\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Custom\0"));      
    ...
    ImGui::Combo(skCrypt("Auto peek style"), &config->misc.autoPeekStyle, skCrypt("Outlined circle\0Filled circle\0Particles\0Sparks\0Gradient in\0Gradient out\0"));
...
void GUI::renderMiscWindow() noexcept
{
    ...
    if (ImGui::BeginPopup(""))
    {
        ...
        ImGui::Checkbox(skCrypt("Reveal enemy chat"), &config->misc.chatRevealer);
        ImGui::Checkbox(skCrypt("Reveal cheats"), &config->misc.cheatRevealer);
        if (config->misc.cheatRevealer)
            if (ImGui::Button(skCrypt("Force Update")))
    ...
    if (ImGui::BeginPopup("")) {
        ...
        ImGui::Checkbox(c_xor("Show Info"), &config->misc.wm.showInfo);
        ImGui::EndPopup();
    }
    ImGui::PopID();
    ...
    ImGui::Combo("ClanTag", &config->clanCfg.mode, "None\0BLACKHOLE\0Clock\0Reverse\0Velocity\0Position\0HP\0\\n Overflow\0gamesense\0Custom\0Stealer\0");
    if (config->clanCfg.mode == 9) {
        ...
        if (ImGui::BeginPopup("##custom")) {
            ...
            ImGui::Combo("Type", &config->clanCfg.custom.mode, "Static\0Rotate\0Rotate Backwards\0Add\0Remove\0");
            if (ImGui::InputText("Name", &config->clanCfg.custom.tag))
                Clan::update(false, true);
            if (ImGui::InputText("Team", &config->clanCfg.custom.teamTag))
                Clan::update(false, true);
            if (ImGui::InputText("Prefix", &config->clanCfg.custom.prefix))
                Clan::update(false, true);
            if (ImGui::InputText("Postfix", &config->clanCfg.custom.postfix))
                Clan::update(false, true);
            ImGui::SliderFloat("##speed", &config->clanCfg.custom.speed, 0.01f, 1.f, "Speed: %.2f", ImGuiSliderFlags_AlwaysClamp | ImGuiSliderFlags_Logarithmic);
            ImGui::Checkbox("Hide Name", &config->clanCfg.custom.hideName);
            ImGui::Separator();
            ImGui::PopItemWidth();
    ...
    if (config->clanCfg.mode == 10) {
        ...
        if (ImGui::BeginPopup("##stealer")) {
            ImGui::PushItemWidth(120.f);
            ImGui::Checkbox("Show All", &config->clanCfg.stealerShowAll);
            ImGui::SameLine();
            if (ImGui::Button("Stop")) {
                Clan::SetStealFromIdx(-1);
                Clan::SetStealEnabled(false);
                Clan::update(true);
            }
            if (!Clan::players.empty() && localPlayer && playerResource)
            {
                if (ImGui::BeginTable("", 3)) {
                    ...
                    for (unsigned int n = 0; n < Clan::players.size(); n++) {

                        if (Clan::players[n].index == localPlayer->index()) continue;

                        auto c = playerResource->getClan(Clan::players[n].index);

                        if (!config->clanCfg.stealerShowAll && c && !c[0]) continue;

                        ImGui::TableNextRow();
                        ImGui::PushID(ImGui::TableGetRowIndex());

                        if (ImGui::TableNextColumn())
                            ImGui::Text("%s", Clan::players[n].name.c_str());
                        ...
                        if (ImGui::TableNextColumn())
                            if (ImGui::Button("Steal")) {
                                Clan::SetStealFromIdx(Clan::players[n].index);
                                Clan::SetStealEnabled(true);
                                Clan::update(true);
                            }
    ...
    if (ImGui::BeginPopup("")) {
        ...
        if (config->misc.spam.spam_picker == 2)
        {
            ...
            ImGui::hotkey2("Did i ask?", config->misc.spam.DidNotAskKey);
        }
        ImGui::EndPopup();
    }
    ...
    if (config->backtrack.fakeLatency)
        ImGui::SliderInt(skCrypt("Amount"), &config->backtrack.fakeLatencyAmount, 0, static_cast<int>(Backtrack::getMaxUnlag() * 1000), "%d ms");
    ImGui::PopItemWidth();
    ...
    ImGui::Combo(c_xor("Hit Sound"), &config->misc.hitSound, skCrypt("None\0BLACKHOLE\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Custom\0"));
    ...
    ImGui::PushID(2);
    ImGui::Combo(c_xor("Kill Sound"), &config->misc.killSound, skCrypt("None\0BLACKHOLE\0Metal\0Gamesense\0Bell\0Glass\0Coins\0Custom\0"));
...
void GUI::renderConfigWindow() noexcept
{
    ...
    if (ImGui::ListBox("", &currentConfig, [](void* data, int idx, const char** out_text) {
        ...
        *out_text = vector[idx].c_str();
        ...
        if (ImGui::BeginPopup(skCrypt("Config to reset"))) {
            ...
            for (int i = 0; i < IM_ARRAYSIZE(names); i++) {
                ...
                if (ImGui::Selectable(names[i])) {
                    switch (i) {
                    case 0: config->reset(); Clan::update(true); SkinChanger::scheduleHudUpdate(); break;
                    ...
                    case 7: config->glow = { }; config->playerGlow = { }; break;
                    ...
                    case 12: config->misc = { };  Clan::update(true); break;
                    }
                }
            }
            ImGui::EndPopup();
        ...
        ImGuiCustom::colorPicker("FX Enabled", config->menu.lineCol);
        if (config->menu.lineCol.enabled)
        {
            ImGui::SliderInt("##amount", &config->menu.N, 10, 1000, "Amount: %i");
            if (ImGui::IsItemHovered()) ImGui::SetTooltip("Higher numbers may lags in menu!");
            ImGui::SliderInt("##distance", &config->menu.lineMaxDist, 10, 2000, "Distance: %i");
            ImGui::SliderFloat("##thick", &config->menu.lineThickness, .1f, 5.f, "Thickness: %.1f");
        }
        ImGui::SliderFloat("##dar", &config->menu.darkness, 0.01f, 1.f, "Darkness: %.3f");
}
...
void GUI::renderGuiStyle() noexcept
{
    ...
    static auto Name = skCrypt("BLACKHOLE");
        ...
        ImGui::BeginChild(skCrypt("##Back"), ImVec2{ 754- 70, 559 }, false);
        {
            ...
            ImGui::BeginChild(skCrypt("##Main"), ImVec2{ 750 - 70, 555 }, false);
            {
                ...
                if (text == " BLACKHOLE")
                {
                    ...
                    {
                        draw->AddText(fonts.tahoma34, 24.f, ImVec2(pos.x + 26.5f + 1, pos.y + 2), Helpers::calculateColor(config->menu.accentColor, 0.5f), skCrypt(" BLACKHOLE"));
                        draw->AddText(fonts.tahoma34, 24.f, ImVec2(pos.x + 26.5f, pos.y + 1), ImColor(255.f, 255.f, 255.f, 255.f), skCrypt(" BLACKHOLE"));
                        ImGui::SameLine();
