...
void Logger::render() noexcept
{
    ...
    for (int i = renderLogs.size() - 1; i >= 0; i--)
    {
        ...
        if (config->misc.loggerOptions.position == 0)
            ...
            interfaces->surface->setTextPosition(interfaces->surface->getScreenSize().first / 2 - interfaces->surface->getTextSize(hooks->nameEsp, std::wstring(renderLogs[i].text.begin(), renderLogs[i].text.end()).c_str()).first / 2, interfaces->surface->getScreenSize().second / 2 + config->misc.loggerOptions.offset + (17 * i) + 1);
...
void Logger::addRenderLog(std::string logText) noexcept
{
    Log log;
    log.time = memory->globalVars->realtime;
    log.text = logText;
