...
static bool isGrenadeWarningEnabled(WeaponId id)
{
	...
	{
	case WeaponId::Flashbang:
		return config->visuals.warning.flash;
		break;
	case WeaponId::HeGrenade:
		return config->visuals.warning.HE;
		break;
	case WeaponId::SmokeGrenade:
		return config->visuals.warning.smoke;
		break;
	case WeaponId::Molotov:
	case WeaponId::IncGrenade:
		return config->visuals.warning.molotov;
		break;
	case WeaponId::Decoy:
		return config->visuals.warning.decoy;
		break;
	default:
		return false;
		break;
...
void NadePrediction::run() noexcept
{
	...
	if (!config->visuals.GPenabled)
		return;
	GameData::Lock lock;
	const auto& local = GameData::local();
	if (!config->visuals.warning.enabled)
		return;
	...
	for (const auto& projectile : GameData::projectiles())
	{
		if (projectile.trajectory.empty() || projectile.exploded || !projectile.Velocity.notNull())
			continue;
		if (config->visuals.warning.enemiesOnly && !projectile.thrownByEnemy)
			continue;
...
{
	if (!config->visuals.GPtrail.enabled)
		return;

	ImVec2 currentPoint{};
	ImVec2 previousPoint{};
	const auto color = Helpers::calculateColor(config->visuals.GPtrail);
...
{
	if (!config->visuals.GPcolision.enabled)
		return;
	// TODO: add 3d box
	ImVec2 currentPoint;
	const auto color = Helpers::calculateColor(config->visuals.GPcolision);
	for (auto& nade : worldPoints)
	{
