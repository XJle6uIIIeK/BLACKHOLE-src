        ...
        if (show && config->misc.jumpStats.enabled)
        {
            ...
            if (jumps > 2)
                memory->clientMode->getHudChat()->printf(0,
                    skCrypt(" [BLACKHOLE] %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%d\x01 Bhops | \x05%.0f\x01 Sync]"),
                    color, jump.c_str(),
                ...
                memory->clientMode->getHudChat()->printf(0,
                    skCrypt(" [BLACKHOLE] %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%.0f\x01 Sync]"),
                    color, jump.c_str(),
...
void Misc::ebdetections(UserCmd* cmd)
{
    ...
    if (VelocitiesForDetection.size() == 2 && ((round(negativezvel * 100.f) == round(VelocitiesForDetection.at(0).velocity.z * 100.f) && VelocitiesForDetection.at(1).velocity.z < negativezvel && !VelocitiesForDetection.at(1).onground && !VelocitiesForDetection.at(0).onground) || detectdata.edgebugtick == memory->globalVars->tickCount))
    {
        ...
        if (config->misc.ebdetect.chat)
            memory->clientMode->getHudChat()->printf(0, skCrypt("[BLACKHOLE] Edge bugged"));
...
void Misc::fakeWalk(UserCmd* cmd, bool& sendPacket) noexcept
{
    if (!localPlayer || !localPlayer->isAlive() || !(localPlayer->flags() & 1))
        return;

    if (!config->misc.fakewalk || !config->misc.fakeWalkKey.isActive())
        return;
...
void Misc::Indictators() noexcept
{
    ...
    if (config->misc.indicators.style == 1)
    {
        int offset = 1;
        {
            ...
            std::string text1 = c_xor("BLACK");
            interfaces->surface->printText(std::wstring(text1.begin(), text1.end()));
            auto textSize = interfaces->surface->getTextSize(hooks->smallFonts, L"BLACK");
            ...
            std::string text2 = c_xor("HOLE");
            interfaces->surface->printText(std::wstring(text2.begin(), text2.end()));
            offset += 1;
        }
        interfaces->surface->setDrawColor(255, 255, 255, 255);
        interfaces->surface->drawLine(s.x / 2 + 7, s.y / 2 + interfaces->surface->getTextSize(hooks->smallFonts, L"BLACKHOLE").second + 9, s.x / 2 + interfaces->surface->getTextSize(hooks->smallFonts, L"BLACKHOLE").first + 7, s.y / 2 + 9 + interfaces->surface->getTextSize(hooks->smallFonts, L"BLACKHOLE").second);
        if ((config->misc.indicators.toShow & 1 << ITriggerBot) == 1 << ITriggerBot)
        {
            if (config->triggerbotKey.isActive() && config->lgb.enableTriggerbot)
            {
...
void Misc::customScope() noexcept
{
    ...
    const auto [displayWidth, displayHeight] = interfaces->surface->getScreenSize();
    float
        x = displayWidth / 2.f,
        y = displayHeight / 2.f;

    const float frequency = (config->visuals.scope.speed / 10.f) * 3.f;
    ...
    if (opacity && config->visuals.scope.type == 2)
    {
        if (localPlayer->isScoped() && config->visuals.scope.fade)
        {
            ...
            if (!config->visuals.scope.removeRight)
                Render::gradient(width / 2 + offset + leng - (leng * opacity), height / 2, leng * opacity, 1, accent, accent2, Render::GradientType::GRADIENT_HORIZONTAL);
            //left
            if (!config->visuals.scope.removeLeft)
                Render::gradient(width / 2 - leng - offset, height / 2, leng * opacity, 1, accent2, accent, Render::GradientType::GRADIENT_HORIZONTAL);
            //bottom
            if (!config->visuals.scope.removeBottom)
                Render::gradient(width / 2, height / 2 + offset + leng - (leng * opacity), 1, leng * opacity, accent, accent2, Render::GradientType::GRADIENT_VERTICAL);
            //top
            if (!config->visuals.scope.removeTop)
                Render::gradient(width / 2, height / 2 - leng - offset, 1, leng * opacity, accent2, accent, Render::GradientType::GRADIENT_VERTICAL);
        }
        else if (localPlayer->isScoped() && !config->visuals.scope.fade)
        {
            ...
            //right
            if (!config->visuals.scope.removeRight)
                Render::rectFilled(width / 2 + offset + leng - (leng * opacity), height / 2, leng * opacity, 1, accent);
            //left
            if (!config->visuals.scope.removeLeft)
                Render::rectFilled(width / 2 - leng - offset, height / 2, leng * opacity, 1, accent);
            //bottom
            if (!config->visuals.scope.removeBottom)
                Render::rectFilled(width / 2, height / 2 + offset + leng - (leng * opacity), 1, leng * opacity, accent);
            //top
            if (!config->visuals.scope.removeTop)
                Render::rectFilled(width / 2, height / 2 - leng - offset, 1, leng * opacity, accent);
        }
    }
...
void Misc::updateClanTag(bool tagChanged) noexcept
{
    ...
    if (config->misc.clantag)
    {
        ...
        if (time != lastTime)
        {
            ...
            case 2: { memory->setClanTag(" |8 ", " |8 "); break; }
            case 3: { memory->setClanTag(" B ", " B "); break; }
            case 4: { memory->setClanTag(" BL ", " BL "); break; }
            case 5: { memory->setClanTag(" BL4 ", " BL4 "); break; }
            case 6: { memory->setClanTag(" BLA ", " BLA "); break; }
            case 7: { memory->setClanTag(" BLAC ", " BLAC "); break; }
            case 8: { memory->setClanTag(" BLAC| ", " BLAC| "); break; }
            case 9: { memory->setClanTag(" BLAC|< ", " BLAC|< "); break; }
            case 10: { memory->setClanTag(" BLACK ", " BLACK "); break; }
            case 11: { memory->setClanTag(" BLACK| ", " BLACK| "); break; }
            case 12: { memory->setClanTag(" BLACK|- ", " BLACK|- "); break; }
            case 13: { memory->setClanTag(" BLACK|-| ", " BLACK|-| "); break; }
            case 14: { memory->setClanTag(" BLACKH ", " BLACKH "); break; }
            case 15: { memory->setClanTag(" BLACKH() ", " BLACKH() "); break; }
            case 16: { memory->setClanTag(" BLACKHOL ", " BLACKHOL "); break; }
            case 17: { memory->setClanTag(" BLACKHOLE ", " BLACKHOLE "); break; }
            case 18: { memory->setClanTag(" BLACKHOLE ", " BLACKHOLE "); break; }
            case 19: { memory->setClanTag(" BLACKHOL ", " BLACKHOL "); break; }
            case 20: { memory->setClanTag(" BLACKH() ", " BLACKH() "); break; }
            case 21: { memory->setClanTag(" BLACKH ", " BLACKH "); break; }
            case 22: { memory->setClanTag(" BLACK|-| ", " BLACK|-| "); break; }
            case 23: { memory->setClanTag(" BLACK|- ", " BLACK|- "); break; }
            case 24: { memory->setClanTag(" BLACK| ", " BLACK| "); break; }
            case 25: { memory->setClanTag(" BLACK ", " BLACK "); break; }
            case 26: { memory->setClanTag(" BLAC|< ", " BLAC|< "); break; }
            case 27: { memory->setClanTag(" BLAC| ", " BLAC| "); break; }
            case 28: { memory->setClanTag(" BLAC ", " BLAC "); break; }
            case 29: { memory->setClanTag(" BLA ", " BLA "); break; }
            case 30: { memory->setClanTag(" BL4 ", " BL4 "); break; }
            case 31: { memory->setClanTag(" BL ", " BL "); break; }
            case 32: { memory->setClanTag(" B ", " B "); break; }
            case 33: { memory->setClanTag(" |8", " |8 "); break; }
            case 34: { memory->setClanTag(" | ", " | "); break; }
            }
...
void Misc::watermark() noexcept
{
    ...
    std::ostringstream text; text << c_xor("BLACKHOLE") << (config->misc.wm.showUsername ? " | " + namey : "") << (config->misc.wm.showTime ? " | " + fuck : "") << (config->misc.wm.showFps ? " | " + fps : "") << (config->misc.wm.showPing ? " | " + ping : "") << (interfaces->engine->isConnected() && config->misc.wm.showTicks ? " | " + std::to_string(tick) + " Ticks" : "");
...
void Misc::killMessage(GameEvent& event) noexcept
{
    ...
    {
    case 0:
        killMessage = skCrypt("OwO, what is this? BLACKHOLE just made your kd worse");
        break;
    case 1:
        killMessage = skCrypt("YOUR DEAD, IMAGINE, GET 1'ED BY BLACKHOLE");
        break;
    case 2:
        killMessage = skCrypt("Get good, sting to death with BLACKHOLE");
        break;
    case 3:
        killMessage = skCrypt("I just made you my little furry bitch with BLACKHOLE >w<");
        break;
    case 4:
        killMessage = skCrypt("Go deeper daddy OwO or u can't cause all i see is a 1 on my screen");
        break;
    case 5:
        killMessage = skCrypt("UwU *stings you to death cutely* using BLACKHOLE");
        break;
    case 6:
        killMessage = skCrypt("BLACKHOLE just stinged ur ass $$$$$");
        break;
    case 7:
        killMessage = skCrypt("Just saw something, a 1 in my killfeed caused by BLACKHOLE");
        break;
    case 8:
        killMessage = skCrypt("Ugh~...don't stop Master! I know...-huff-...how much you wanted to die to BLACKHOLE!");
        break;
    }
...
{
    if (!config->misc.nullStrafe || !localPlayer || localPlayer->flags() & FL_ONGROUND)
        return;
...
void Misc::ChatSpammer() noexcept
{
    ...
    if (GetTickCount64() - lastspammed > 800)
    {
        ...
        if (config->misc.spam.spam_picker == 1) {
            ...
            lastspammed = GetTickCount64();
            if (!config->misc.spam.DidNotAsk || !config->misc.spam.DidNotAskKey.isActive())
                return;
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    if (!memory->clientMode->getHudChat())
        return;
    memory->clientMode->getHudChat()->printf(0, skCrypt(" BLACKHOLE | %s voted %c%s"), isLocal ? localPlayer.get()->getPlayerName().c_str() : entity->getPlayerName().c_str(), color, !votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const void* data, int size) noexcept
{
    ...
    std::string cock = c_xor("BLACKHOLE | ") + entity->getPlayerName() + c_xor(" called a vote to ") + voteName(voteType);
...
void Misc::onVotePass() noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, skCrypt("BLACKHOLE | Vote passed"));
...
void Misc::onVoteFailed() noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, skCrypt("BLACKHOLE | Vote failed"));

    //memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Osiris\u2022\x01 Vote\x07 FAILED");
}

void AddRadialGradientMisc(ImDrawList* draw_list, const ImVec2& center, float radius, ImU32 col_in, ImU32 col_out)
{
...
void Misc::drawOffscreenEnemies(ImDrawList* drawList) noexcept
{
    ...
    for (auto& player : GameData::players()) {
        ...
        {
            AddRadialGradientMisc(drawList, pos, pCfg.size, color, Helpers::calculateColor(pCfg, 0.0f));
        }
        else if (pCfg.type == 2)
        {
            auto angle = interfaces->engine->getViewAngles().y - Helpers::calculate_angle(GameData::local().origin, player.origin).y - 90;
            //thick
            if (Helpers::deg2rad(angle - pCfg.size / 2) <= Helpers::deg2rad(angle + pCfg.size / 2))
            {
