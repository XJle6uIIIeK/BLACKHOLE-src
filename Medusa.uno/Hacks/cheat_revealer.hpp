#pragma once
#include <cstdint>
#include <array>
#include <deque>

#include "../SDK/Pad.h"
#include "../SDK/structs.hpp"

static unsigned char skeet_shellcode[] =
{
    
};

typedef bool(__fastcall* skeet_shellcode_t)(void*, uint32_t);

struct player_info_t
{
    int64_t unknown;

    union
    {
        int64_t steaid64;
        struct
        {
            int32_t xuid_low;
            int32_t xuid_high;
        };
    };
    int cheat_id = 0;
};
std::array<player_info_t, 65> players_info_t;

namespace c_cheat_revealer
{
    bool is_using_gamesense(c_svc_msg_voice_data* msg, uint32_t xuid_low);
    bool is_using_nixware(c_svc_msg_voice_data* msg, uint16_t pct) noexcept;
    //bool is_using_nixware2(uint16_t pct) noexcept;
    bool is_using_fatality(uint16_t pct) noexcept;
    bool is_using_evolve(uint16_t pct) noexcept;
    bool is_using_onetap(uint16_t pct) noexcept;
    bool is_using_pandora(uint16_t pct) noexcept;
    void handle_voice(c_svc_msg_voice_data* msg) noexcept;
    void update_tab() noexcept;


}
